  ...
  subroutine fit_fermi(m,x,y,beta)
  USE kinds,         ONLY : DP
  ...
  real(kind=dp), intent(inout):: beta(n) ! initial guess on the entry
  !
  integer  :: ldfjac
  ...
  xdat(1:m) = x(1:m)
  ydat(1:m) = y(1:m) ! 1/fe = exp(beta*x) + 1
  !ydat(1:m) = 1.0_dp/y(1:m) ! 1/fe = exp(beta*x) + 1

  call fermi_func(m,n,beta,fvec,fjac,ldfjac,iflag)

  tol = 1.e-6_dp

  call lmder1 ( fermi_func, m, n, beta, fvec, fjac, ldfjac, tol, info )

  write(stdout,*) 'beta=',beta(1),info
  !do i = 1, m
  !  write(stdout,*) xdat(i),ydat(i),fvec(i)
  !enddo
  ...
  !---------------------------
  subroutine fermi_func(m,n,beta,fvec,fjac,ldfjac,iflag)
  USE kinds,         ONLY : DP
  use fit
  implicit none
  integer,       intent(in)    :: m,n,ldfjac,iflag
  real(kind=dp), intent(in)    :: beta(n)
  real(kind=dp), intent(inout) :: fvec(m)
  ...
  if (iflag==0) then
    ...
    fvec(1:m) = 1.0_dp/(exp(beta(1) * xdat(1:m)) +1.0) - ydat(1:m)
    !fvec(1:m) = 1.0_dp/exp(beta(1) * xdat(1:m) +1.0) - ydat(1:m)

  else if (iflag==2) then
    ! gradient
      occ(1:m) = 1.0_dp/(exp(beta(1) * xdat(1:m)) +1.0)
    do i = 1, m
      fjac(i,1) = - occ(i) * occ(i) * exp(beta(1) * xdat(i)) * xdat(i)
    enddo
  else
